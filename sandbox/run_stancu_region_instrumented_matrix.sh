...
run_mode() {
  ...
  echo "== ${label} =="
  if [[ "${platform}" == "Darwin" ]]; then
    SAFEZONE_ROOTS_MODE="${roots_mode}" /usr/bin/time -l "${binary}" "${mode}" > "${run_log}" 2> "${time_log}"
  else
    SAFEZONE_ROOTS_MODE="${roots_mode}" /usr/bin/time -v "${binary}" "${mode}" > "${run_log}" 2> "${time_log}"
  fi

  max_rss_bytes=$(read_max_rss_bytes "${time_log}")
  grep "^RESULT name=stancu-transactions-" "${run_log}"
  echo "STANCU_RSS_RESULT label=${label} mode=${mode} max_rss_bytes=${max_rss_bytes}"
