...
if [[ -z "${STANCU_MODES:-}" && ( "${include_controls}" == "1" || "${include_controls}" == "true" || "${include_controls}" == "yes" ) ]]; then
  modes+=(current-safezone unsafezone-hp rift-hp rift-streaming rift-checked-direct-epoch rift-checked-direct-epoch-legacy rift-checked-safezone-direct-epoch)
fi
...
for selected_mode in "${modes[@]}"; do
  case "${selected_mode}" in
    ...
    rift-checked-direct-epoch-open-handle)
      run_mode "rift-checked-direct-epoch-open-handle" "rift-checked-direct-epoch-open-handle" "0"
      ;;
    rift-checked-direct-epoch-legacy)
      run_mode "rift-checked-direct-epoch-legacy" "rift-checked-direct-epoch-legacy" "0"
      ;;
