...
real LogHub files. The first real BGL rows are correctness-valid and show
modest throughput/RSS/fixed-memory value, but not a flagship GC-heavy case.
...
The current conclusion is therefore "real-input modest throughput/RSS/tail
evidence," not "GC-heavy real-data case study." Next benchmark search should
continue to DSPBench local kernels, real RIoTBench-style inputs, or another
larger log/NDJSON workload with heavier per-record object materialization.
