...
run_case() {
  ...
  if ! grep -q "^RESULT name=loghub-top-templates-${binary_mode} " "${run_log}"; then
    ...
    echo "LOGHUB_TOP_RESULT mode=${mode} heap_cap=${heap_cap} status=failed exit_status=${command_status} max_rss_bytes=${max_rss_bytes}" >&2
    write_failed_row "${mode}" "${heap_cap}" "failed:${command_status}" "${max_rss_bytes}"
    return 0
  fi

  max_rss_bytes=$(read_max_rss_bytes "${time_log}")
  grep "^RESULT name=loghub-top-templates-${binary_mode} " "${run_log}"
  echo "LOGHUB_TOP_RSS_RESULT mode=${mode} heap_cap=${heap_cap} max_rss_bytes=${max_rss_bytes}"
  write_result_row "${mode}" "${heap_cap}" "ok" "${binary_mode}" "${run_log}" "${max_rss_bytes}"
}
