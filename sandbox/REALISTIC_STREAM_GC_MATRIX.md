...
| Yak `graphreal` SNAP graph replay | real preloaded SNAP Twitter ego and LiveJournal edge lists, replayed as epoch-local edge-update objects | Twitter ego 1M/2M proved the path. The LiveJournal topology follow-up is stronger and less ambiguous. Older topology rows at 50M show `gc-heap` `1618.105 ms` with `273.410 ms` GC and `2.76 GB` RSS, while per-epoch rows use about `1.53 GB` RSS and remove timed GC. `RiftRegion.epoch { ... }` now exposes the direct checked linked-epoch topology as reusable API. The API-backed reruns confirm it: at 10M, direct checked epoch is `212.691/229.532 ms` scoped/stream versus heap `317.779 ms`; at 50M with the same 10 x 5M topology, API-backed `checked-epoch-scoped` is `1055.958 ms` with `1.53 GB` RSS versus heap `1604.811 ms`, `288.801 ms` GC, and `2.76 GB` RSS. Checked page-token rows still beat heap (`1403.878/1457.811 ms` in the older topology run) but are the wrong topology for Yak. | Current top prior-work-shaped real-input ladder row. It is promising because LiveJournal has real graph provenance, material heap GC/RSS pressure, stable region wins, and checked direct epoch topology beats heap strongly. The reusable checked epoch API is implemented; `EpochBuffer` remains a correct but slower append/drain abstraction. It is still not exact Yak/GraphChi artifact reproduction. |
...
| LogHub / LogPAI BGL and HDFS v1 | real file-backed system logs | BGL loads `4747963` lines. At 1M, heap GC appears in every run (`99-157 ms`) but remains a small share of roughly `5.6 s` elapsed. HDFS v1 adds a larger `11175629`-line Hadoop log; its strongest row is q2 at 1M, where checked scoped page-token is `7871.856 ms` vs heap `8227.369 ms`, removes heap median GC `92.659 ms`, and lowers RSS from `408666112` to `394969088`. | Keep as real-input modest-win/control evidence. HDFS q2 is the strongest checked LogHub row so far, but heap GC remains about 1% of elapsed, so it is not the missing GC-heavy flagship. |
...
| LogHub BGL/HDFS q1/q2 file-backed | The real BGL and HDFS logs create many line/token objects and trigger heap GC more steadily than GH Archive byte-slice rows. At 1M BGL lines, heap GC is `99.271 ms` for q1 and `157.198 ms` for q2, but elapsed is still around `5.6 s`. At 1M HDFS lines, q2 heap GC is `92.659 ms` inside `8227.369 ms`, while checked scoped page-token is `7871.856 ms`. Parser/line/token CPU still dominates. |
| Yak `graphreal` SNAP graph replay | Twitter ego first exposed the path; LiveJournal makes the signal stronger. At 50M replayed real edges, `gc-heap` spends `273.410 ms` median timed GC inside `1618.105 ms` elapsed and uses `2761261056` RSS. The fastest low-RSS safe checked topology is `checked-epoch-scoped` at `1069.241 ms`; `region-scoped-rooted` is `1256.538 ms`; `region-stream-rootless` is `1345.479 ms`. Whole-run checked scoped is fastest at `1048.751 ms` but uses `2977742848` RSS. Reusable `EpochBuffer` scoped is positive but slower (`1343.071 ms` vs heap `1514.313 ms`, and vs linked epoch `1022.643 ms`). This is now the best real-input prior-work-shaped row, although it remains a local graph replay rather than exact GraphChi/Yak. |
...
Latest Yak real graph control:

| Query | Scale | Heap | Best region row | Heap GC | Output/checksum | Decision |
|---|---:|---:|---:|---:|---:|---|
| `graphreal` | 1M replayed SNAP Twitter ego edges | `gc-heap` `31.202 ms`, RSS `74792960` | `region-scoped-rooted` `23.919 ms`; `region-stream-rootless` `24.315 ms`; `region-hp-rootless` `24.383 ms`, RSS about `42.2 MB` | median `5.267 ms` | matched | First real-input Yak-shaped row; superseded by LiveJournal for scale. |
| `graphreal` | 2M replayed SNAP Twitter ego edges | `gc-heap` `63.454 ms`, RSS `172523520` | `region-scoped-rooted` `47.967 ms`; `region-hp-rootless` `48.474 ms`; `region-stream-rootless` `49.557 ms`, RSS about `65 MB` | median `0.000 ms`; one heap run `16.857 ms` | matched | Throughput/RSS/tail direction holds; superseded by LiveJournal for scale. |
| `graphreal` | 5M replayed SNAP LiveJournal edges | `gc-heap` `173.832 ms`, RSS `322699264` | `region-stream-rootless` `130.716 ms`; `region-scoped-rooted` `139.264 ms`; `checked-region-scoped` `146.845 ms`; `checked-region-stream` `150.667 ms`, RSS about `190 MB` | median `33.456 ms` | matched | Strong real graph row: checked and trusted/rooted region modes beat heap and cut RSS materially. |
| `graphreal` | 10M replayed SNAP LiveJournal edges | `gc-heap` `319.560 ms`, RSS `605569024` | `region-scoped-rooted` `249.543 ms`; `region-stream-rootless` `257.299 ms`; `checked-region-scoped` `272.072 ms`; `checked-region-stream` `286.336 ms`, RSS about `328.6 MB` | median `51.987 ms` | matched | Checked page-token rows also beat heap; superseded by the 50M scale row as the current representative Yak-shaped result. |
| `graphreal` | 50M replayed SNAP LiveJournal edges | API-backed `gc-heap` `1604.811 ms`, RSS `2759835648`; older topology heap `1618.105 ms`, RSS `2761261056` | API-backed direct epoch: `checked-epoch-scoped` `1055.958 ms`, `checked-epoch-stream` `1101.001 ms`, RSS about `1.53 GB`; older low-RSS epoch rows `1069.241/1113.261 ms`; reusable `checked-epoch-buffer-scoped` `1361.752 ms` in the API-backed rerun; whole-run checked scoped older row `1048.751 ms`, RSS `2977742848` | API-backed heap GC `288.801 ms`; older topology heap GC `273.410 ms` | matched | Less short-run dominated scale row; checked direct epoch topology is the right Yak shape and now reusable through `RiftRegion.epoch`. `EpochBuffer` is correct but not the fastest epoch lowering. |
...
Latest LogHub BGL/HDFS control:
...
| q1-tokens | 1M real HDFS lines / `13495462` line+token records | `8059.155 ms`, RSS `408551424` | safezone-improved-32k `7919.352 ms`; checked scoped page-token `7923.079 ms`; Streaming `7938.369 ms` | median `139.906 ms`, max `191.623 ms`, 3/3 runs | `13495462` | HDFS q1 is a modest real-input throughput/RSS/GC-tail win for scoped/checked rows; not flagship because parser/token CPU dominates. |
| q2-window-counts | 1M real HDFS lines / `13495462` line+token records | `8227.369 ms`, RSS `408666112` | checked scoped page-token `7871.856 ms`, RSS `394969088`; Streaming `7871.713 ms`, RSS `356073472` | median `92.659 ms`, max `104.790 ms`, 3/3 runs | `41` | Strongest checked LogHub row so far: about 4.3% elapsed win plus GC/RSS reduction, still not GC-heavy proof. |
| q3-template-session | 1M real HDFS lines / `5913054` region/session records | `8460.928 ms`, RSS `408666112` | safezone-improved-32k `8360.487 ms`, RSS `319864832`; Streaming `8439.205 ms`, RSS `319143936`; checked scoped page-token `8506.708 ms` | heap median `81.910 ms`, max `88.603 ms`, 3/3 runs | `283857` | RSS/GC-tail evidence, not a checked elapsed win; richer template/session state still leaves CPU dominant. |
...
3. Another real log/NDJSON workload:
   - use LogHub BGL/HDFS as the baseline log controls;
   - try Thunderbird, Spark, Windows, Android, OpenSSH, or GDELT-like event
     files only if they materialize more per-record objects or force steadier
     heap GC than BGL/HDFS.
