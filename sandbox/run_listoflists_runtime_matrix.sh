...
modes=(${(z)${LISTBENCH_RUNTIME_MODES:-"heap improved-safezone checked-region-listoflists-builder"}})
if [[ -z "${LISTBENCH_RUNTIME_MODES:-}" && ( "${include_controls}" == "1" || "${include_controls}" == "true" || "${include_controls}" == "yes" ) ]]; then
...
for selected_mode in "${modes[@]}"; do
  case "${selected_mode}" in
    ...
    rift-hp)
      ...
      ;;
    checked-region-listoflists-builder)
      run_mode "Checked ListOfLists builder" "checked-region-listoflists-builder"
      ;;
    *)
      echo "unknown LISTBENCH_RUNTIME_MODES entry: ${selected_mode}" >&2
      exit 1
      ;;
  esac
done
