...
output_dir=${CHECKED_SWR_OUTPUT_DIR:-"/tmp/checked-stream-window-rank"}
...
if [[ "${build}" != "0" ]]; then
  sbt \
    ...
    "set Compile / mainClass := Some(\"CheckedStreamWindowRankMatrix\")" \
    nativeLink
fi

binary=${CHECKED_SWR_BINARY:-}
if [[ -z "${binary}" ]]; then
  binary=$(find sandbox/.3-next/target -path "*/native/CheckedStreamWindowRankMatrix" -type f -perm -111 -print | sort | tail -n 1)
fi

if [[ -z "${binary}" || ! -x "${binary}" ]]; then
  echo "missing CheckedStreamWindowRankMatrix native binary; set CHECKED_SWR_BINARY or enable CHECKED_SWR_BUILD" >&2
  exit 1
fi
...
write_result_row() {
  ...
  line=$(grep "^RESULT name=checked-stream-window-rank-" "${run_log}" | tail -n 1)
  fields=()
...
run_mode() {
  ...
  if ! grep -q "^RESULT name=checked-stream-window-rank-" "${run_log}"; then
    cat "${run_log}" >&2
    cat "${time_log}" >&2
    exit "${command_status}"
  fi
