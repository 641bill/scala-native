...
object YakRegionConfig {
  ...
  val graphInputMode: String = {
    val value = BenchmarkInputSupport.envString("YAK_GRAPH_INPUT_MODE")
      ...
      value match {
        ...
        case other =>
          throw new IllegalArgumentException(
            s"unknown YAK_GRAPH_INPUT_MODE '$other'; expected preloaded or streaming-file"
          )
      }
  }
  val graphStreamingInput: Boolean = graphInputMode == "streaming-file"
  val textInputPath: String = BenchmarkInputSupport.envString("YAK_TEXT_INPUT")
...
object YakRegionMatrixHelpers {
  ...
  private final class RealTextInput(
      val keys: Array[Int],
  ...
  private object RealGraphInput {
    ...
    final class EdgeSource(limit: Long, vertices: Int) {
      private val cfg = YakRegionConfig
      private val reader = BenchmarkInputSupport.openByteLines(cfg.graphInputPath)
      ...
      def nextEdge(): Boolean =
        if (emitted >= limit) false
        else {
          var length = reader.readLine()
          while (length >= 0) {
            val packed = parseEdge(reader.bytes, length, vertices)
            if (packed >= 0L) {
              src = (packed >>> 32).toInt
              dst = packed.toInt
              emitted += 1L
    ...
    def openEdgeSource(): EdgeSource = {
      val cfg = YakRegionConfig
      if (cfg.graphInputPath.isEmpty)
        throw new IllegalArgumentException(
          "YAK_WORKLOAD=graphreal requires YAK_GRAPH_INPUT"
        )
      val limit = cfg.epochs.toLong * cfg.graphInputEdgesPerEpoch.toLong
      new EdgeSource(limit, cfg.graphInputVertices)
    }
    ...
    def requireNextEdge(
      ...
      if (!source.nextEdge())
        throw new IllegalArgumentException(
          s"YAK_GRAPH_INPUT '${YakRegionConfig.graphInputPath}' ended before streaming graphreal could read epoch=$epoch edge=$edge"
        )
  ...
  private object RealTextInput {
    private val TitlePattern = "Title=\"".getBytes("UTF-8")
    private val BodyPattern = "Body=\"".getBytes("UTF-8")
    private var cachedPath: String = ""
    private var cachedTokenLimit: Int = 0
    private var cachedKeySpace: Int = 0
    private var cached: RealTextInput = null
    ...
    private def matchesAt(
      ...
      if (offset + pattern.length > length) false
      else {
        var i = 0
        var ok = true
        while (i < pattern.length && ok) {
          ok = bytes(offset + i) == pattern(i)
          i += 1
        }
        ok
      }
    ...
    private def tokenByte(byte: Int): Boolean =
      (byte >= 'A' && byte <= 'Z') ||
        (byte >= 'a' && byte <= 'z') ||
        (byte >= '0' && byte <= '9')

    private def lowerAscii(byte: Int): Int =
      if (byte >= 'A' && byte <= 'Z') byte + 32 else byte

    private def scanAttribute(
        bytes: Array[Byte],
        length: Int,
        pattern: Array[Byte],
        keys: Array[Int],
      ...
      while (i < length && count < keys.length) {
        if (matchesAt(bytes, length, i, pattern)) {
          i += pattern.length
          while (i < length && bytes(i) != '"'.toByte && count < keys.length) {
            while (
              i < length && bytes(i) != '"'.toByte &&
              !tokenByte(bytes(i) & 0xff)
            ) i += 1
            var hash = 0x811c9dc5
            var tokenLength = 0
            while (
              ...
              hash ^= lowerAscii(bytes(i) & 0xff)
              hash *= 0x01000193
              tokenLength += 1
              i += 1
            }
            if (tokenLength >= 3) {
              keys(count) = BenchmarkInputSupport.positiveModulo(hash, keySpace)
              weights(count) = math.min(tokenLength, 16)
              count += 1
            }
          }
          return count
    ...
    def load(): RealTextInput = {
      ...
      if (
        cached != null &&
        cachedPath == cfg.textInputPath &&
        cachedTokenLimit == cfg.textInputTokens &&
        cachedKeySpace == cfg.keySpace
      ) return cached

      val keys = new Array[Int](cfg.textInputTokens)
      val weights = new Array[Int](cfg.textInputTokens)
      val reader = BenchmarkInputSupport.openByteLines(cfg.textInputPath)
      var count = 0
      try {
        var length = reader.readLine()
        while (length >= 0 && count < cfg.textInputTokens) {
          if (length > 0) {
            val bytes = reader.bytes
            count =
              scanAttribute(
      ...
      if (count <= 0)
        throw new IllegalArgumentException(
          s"YAK_TEXT_INPUT '${cfg.textInputPath}' did not contain usable title/body tokens"
        )

      val outKeys =
        if (count == keys.length) keys
        else java.util.Arrays.copyOf(keys, count)
      val outWeights =
        if (count == weights.length) weights
        else java.util.Arrays.copyOf(weights, count)
      cachedPath = cfg.textInputPath
      cachedTokenLimit = cfg.textInputTokens
      cachedKeySpace = cfg.keySpace
      cached =
  ...
  def runHeapOrRiftRealTextTopWord(modeName: String): Long = {
    val cfg = YakRegionConfig
    ...
    var epoch = 0
    try {
      while (epochStart < input.tokenCount) {
        val epochEnd = math.min(input.tokenCount, epochStart + tokensPerEpoch)
        val region = mode.beginEpoch()
        var records: WordRecord = null
        var i = epochStart
        while (i < epochEnd) {
  ...
  def runHeapRealTextTopWordEpochTopK(): Long = {
    ...
    while (epochStart < input.tokenCount) {
      ...
      while (i < epochEnd) {
        val record =
          new WordRecord(input.keys(i), input.weights(i), true, records)
        if (records == null) tail = record
        records = record
        retainedCount += 1
        localCounts(record.key) += record.weight
        globalCounts(record.key) =
          (globalCounts(record.key) + record.weight.toLong) & 0xffffffffL
        i += 1
      }
  ...
  def runHeapOrRiftGraphRealStreaming(modeName: String): Long = {
    val cfg = YakRegionConfig
    val source = RealGraphInput.openEdgeSource()
    val mode = new ModeState(modeName)
    val values = new Array[Long](cfg.graphInputVertices)
    var vertex = 0
    while (vertex < values.length) {
      values(vertex) = mix(vertex + 131).toLong & 0xffffL
      vertex += 1
    ...
    try {
      while (epoch < cfg.epochs) {
        ...
        var edge = 0
        while (edge < cfg.graphInputEdgesPerEpoch) {
          RealGraphInput.requireNextEdge(source, epoch, edge)
          val src = source.src
          val dst = source.dst
          val delta = ((src ^ dst ^ epoch ^ edge) & 31) - 15
          updates = mode.allocEdgeUpdate(region, src, dst, delta, updates)
  ...
  def runCheckedRealTextTopWordEpoch(safeZoneBackend: Boolean): Long = {
    val cfg = YakRegionConfig
    val input = RealTextInput.load()
    val globalCounts = new Array[Long](cfg.keySpace)
    val localCounts = new Array[Int](cfg.keySpace)
    ...
    def run()(using stream: RiftRegion.StreamingRegion^): Unit = {
      ...
      while (epochStart < input.tokenCount) {
        val currentEpoch = epoch
        val currentStart = epochStart
        val currentEnd = math.min(input.tokenCount, currentStart + tokensPerEpoch)
        RiftRegion.epoch { region ?=>
          final class CheckedWordRecord(
              val key: Int,
              val weight: Int,
          ...
          while (i < currentEnd) {
            records =
              RiftRegion.allocOpen(
                new CheckedWordRecord(input.keys(i), input.weights(i), records)
              )
            i += 1
          }
  ...
  def runCheckedRealTextTopWordEpochTopK(safeZoneBackend: Boolean): Long = {
    val cfg = YakRegionConfig
    val input = RealTextInput.load()
    val globalCounts = new Array[Long](cfg.keySpace)
    val tokensPerEpoch = math.max(1, cfg.textInputTokensPerEpoch)
    ...
    def run()(using stream: RiftRegion.StreamingRegion^): Unit = {
      ...
      while (epochStart < input.tokenCount) {
        ...
        RiftRegion.epoch { region ?=>
          ...
          var i = currentStart
          while (i < currentEnd) {
            val key = input.keys(i)
            val weight = input.weights(i)
            val record =
              RiftRegion.allocOpen(new CheckedWordRecord(key, weight, records))
            if (records == null) tail = record
            records = record
            retainedCount += 1
            globalCounts(key) = (globalCounts(key) + weight.toLong) & 0xffffffffL
  ...
  private def runCheckedGraphRealLinkedEpochStreaming(
      source: RealGraphInput.EdgeSource,
      values: Array[Long],
      currentEpoch: Int
  )(using region: RiftRegion.OpenStreamingRegion^): Unit = {
    val cfg = YakRegionConfig
    ...
    var edge = 0
    while (edge < cfg.graphInputEdgesPerEpoch) {
      RealGraphInput.requireNextEdge(source, currentEpoch, edge)
      val src = source.src
      val dst = source.dst
      val delta = ((src ^ dst ^ currentEpoch ^ edge) & 31) - 15
      updates =
  ...
  private def runCheckedGraphRealLinkedEpochStreamingHandle(
      source: RealGraphInput.EdgeSource,
      values: Array[Long],
      currentEpoch: Int
  )(using region: RiftOpenStreamingHandle^): Unit = {
  ...
  def runCheckedGraphRealEpochStreaming(safeZoneBackend: Boolean): Long = {
    val cfg = YakRegionConfig
    val source = RealGraphInput.openEdgeSource()
    val values = new Array[Long](cfg.graphInputVertices)
    var vertex = 0
      ...
      if (safeZoneBackend) {
        RiftRegion.streamingSafeZone { stream ?=>
          ...
          while (epoch < cfg.epochs) {
            val currentEpoch = epoch
            RiftRegion.epoch { region ?=>
              runCheckedGraphRealLinkedEpochStreaming(
                source,
                values,
                currentEpoch
              )
        ...
        RiftRegion.streamingOpenHandle { stream ?=>
          ...
          while (epoch < cfg.epochs) {
            val currentEpoch = epoch
            RiftRegion.resetOpenHandle {
              runCheckedGraphRealLinkedEpochStreamingHandle(
                source,
                values,
                currentEpoch
              )
  ...
  def runSafeZoneRealTextTopWord(): Long = {
    val cfg = YakRegionConfig
    val input = RealTextInput.load()
    val globalCounts = new Array[Long](cfg.keySpace)
    val localCounts = new Array[Int](cfg.keySpace)
      ...
      SafeZone { sz ?=>
        ...
        while (i < currentEnd) {
          records =
            SafeZoneAllocator.allocate(
              sz,
              new SZWordRecord(input.keys(i), input.weights(i), records)
            )
          i += 1
        }
  ...
  def runSafeZoneGraphRealStreaming(): Long = {
    val cfg = YakRegionConfig
    val source = RealGraphInput.openEdgeSource()
    val values = new Array[Long](cfg.graphInputVertices)
    var vertex = 0
  ...
  private def runWorkload(mode: String, workload: String): WorkloadResult =
    workload match {
      ...
      case "graphreal" =>
        val cfg = YakRegionConfig
        val checksum =
          if (cfg.graphStreamingInput && mode == "safezone")
            runSafeZoneGraphRealStreaming()
          else if (mode == "safezone") runSafeZoneGraphReal()
          else if (
            cfg.graphStreamingInput &&
              (mode == "checked-region-stream" ||
                ...
                mode == "checked-whole-run-scoped" ||
                mode == "checked-epoch-buffer-stream" ||
                mode == "checked-epoch-buffer-scoped")
          )
            throw new IllegalArgumentException(
              "YAK_GRAPH_INPUT_MODE=streaming-file is wired for heap, region-scoped-rooted, checked-epoch-stream, and checked-epoch-scoped; use preloaded mode for page-token, whole-run, or buffer graphreal controls"
            )
          else if (mode == "checked-epoch-stream" && cfg.graphStreamingInput)
            runCheckedGraphRealEpochStreaming(false)
          else if (mode == "checked-epoch-scoped" && cfg.graphStreamingInput)
            runCheckedGraphRealEpochStreaming(true)
          else if (
            mode == "checked-region-stream" ||
            mode == "checked-page-token-stream"
          ...
          else if (cfg.graphStreamingInput)
            runHeapOrRiftGraphRealStreaming(mode)
      ...
      case other =>
        throw new IllegalArgumentException(
          s"unknown Yak workload '$other'; expected wordcount, graphstep, sort, topword, topwordreal, graphchi, graphreal, or promotion"
        )
    }
  ...
  private def controlSlots(workload: String): Long = {
    ...
    workload match {
      ...
      case "topwordreal" => cfg.keySpace.toLong * 3L
...
@main def YakRegionMatrix(
  ...
  try {
    workload match {
      ...
      case "wordcount" | "graphstep" | "sort" | "topword" | "topwordreal" |
          "graphchi" | "graphreal" | "promotion" =>
        YakRegionMatrixHelpers.runBenchmark(mode, workload)
      case other =>
        throw new IllegalArgumentException(
          s"unknown Yak workload '$other'; expected wordcount, graphstep, sort, topword, topwordreal, graphchi, graphreal, promotion, or all"
        )
    }
  } finally {
