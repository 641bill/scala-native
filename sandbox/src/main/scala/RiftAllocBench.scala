...
final class BenchNode(val value: Int, val next: BenchNode)

object RiftAllocBench {
  private final val DefaultWarmups = 2
  private final val DefaultRuns = 5
  private final val DefaultCount = 200000

  private def buildHeapList(count: Int): BenchNode = {
    var i = 0
    var head: BenchNode = null
    while (i < count) {
      head = new BenchNode(i, head)
      i += 1
    }
    head
  }

  private def buildRiftList(region: RiftRegion, count: Int): BenchNode = {
    var i = 0
    var head: BenchNode = null
    while (i < count) {
      head = region.alloc(new BenchNode(i, head))
      i += 1
    }
    head
  }

  private def checksum(head: BenchNode): Long = {
    var sum = 0L
    var node = head
    while (node != null) {
  ...
  private def runOnce(mode: String, count: Int): Long =
    if (mode == "heap") checksum(buildHeapList(count))
    else {
      val region = RiftRegion.open(parseKind(mode))
