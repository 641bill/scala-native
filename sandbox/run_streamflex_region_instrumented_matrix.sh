...
if [[ -z "${STREAMFLEX_MODES:-}" && ( "${include_controls}" == "1" || "${include_controls}" == "true" || "${include_controls}" == "yes" ) ]]; then
  modes+=(current-safezone unsafezone-hp rift-hp rift-streaming rift-checked-direct-epoch rift-checked-safezone-direct-epoch rift-checked-epoch-buffer rift-checked-safezone-epoch-buffer rift-checked-transaction-region)
fi
