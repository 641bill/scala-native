...
run_case() {
  ...
  if ! grep -q "^RESULT name=loghub-${query}-${binary_mode} " "${run_log}"; then
    ...
    echo "LOGHUB_RESULT query=${query} mode=${mode} heap_cap=${heap_cap} status=failed exit_status=${command_status} max_rss_bytes=${max_rss_bytes}" >&2
  ...
  echo "LOGHUB_RSS_RESULT query=${query} mode=${mode} heap_cap=${heap_cap} max_rss_bytes=${max_rss_bytes}"
