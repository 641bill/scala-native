...
run_mode() {
  ...
  echo "== ${label} =="
  if [[ "${platform}" == "Darwin" ]]; then
    SAFEZONE_ROOTS_MODE="${roots_mode}" /usr/bin/time -l "${binary}" "${mode}" "${operator}" > "${run_log}" 2> "${time_log}"
  else
    SAFEZONE_ROOTS_MODE="${roots_mode}" /usr/bin/time -v "${binary}" "${mode}" "${operator}" > "${run_log}" 2> "${time_log}"
  fi
