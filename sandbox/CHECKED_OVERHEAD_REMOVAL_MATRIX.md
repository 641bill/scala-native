...
| Allocation lowering / object construction | `ObjectAllocationLoweringMatrix` removes stream-window/query traversal from the comparison; page-token `allocOpen`, region-open cached stats, and the current-slab zeroed cache remove several runtime checks/loads from operator-owned/final-clean paths. | The current-slab zeroed cache is the first post-profile allocation-body cleanup that beats the object-fast focused row. Remaining cost is object construction, mandatory zero/init where slabs are dirty, allocator body, append/linking, and query traversal. Further wins need layout/object-construction work or safe zero-elision proof, not more lifecycle bookkeeping. |
