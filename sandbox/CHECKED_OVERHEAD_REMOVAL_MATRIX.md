...
| Allocation lowering / object construction | `ObjectAllocationLoweringMatrix` removes stream-window/query traversal from the comparison; page-token `allocOpen` and the region-open cached stats flag remove several runtime checks from operator-owned/final-clean paths. | The latest cached-stats cleanup removes the sampled stats helper but does not beat the prior focused object-fast allocation result. Remaining cost is object construction, zero/init, allocation body, append/linking, and query traversal. Further wins need layout/object-construction work or same-shape operator summaries, not more `checkOpen`/stats-check cleanup. |
