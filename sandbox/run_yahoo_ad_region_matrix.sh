...
output_dir=${YAHOO_AD_OUTPUT_DIR:-"/tmp/yahoo-ad-region-matrix"}
summary=${YAHOO_AD_SUMMARY:-"${output_dir}/summary.tsv"}
build=${YAHOO_AD_BUILD:-1}
platform=$(uname -s)
modes=(${(z)${YAHOO_AD_MODES:-"heap safezone-current safezone-improved rift-hp rift-streaming"}})
...
if [[ "${build}" != "0" ]]; then
  sbt \
    ...
    "set Compile / mainClass := Some(\"YahooAdRegionMatrix\")" \
    nativeLink
fi

binary=${YAHOO_AD_BINARY:-}
if [[ -z "${binary}" ]]; then
  binary=$(find sandbox/.3-next/target -path "*/native/YahooAdRegionMatrix" -type f -perm -111 -print | sort | tail -n 1)
fi

if [[ -z "${binary}" || ! -x "${binary}" ]]; then
  echo "missing YahooAdRegionMatrix native binary; set YAHOO_AD_BINARY or enable YAHOO_AD_BUILD" >&2
  exit 1
fi
...
write_result_row() {
  ...
  line=$(grep "^RESULT name=yahoo-ad-${query}-${binary_mode} " "${run_log}" | tail -n 1)
  fields=()
...
run_case() {
  ...
  if ! grep -q "^RESULT name=yahoo-ad-${query}-${binary_mode} " "${run_log}"; then
    cat "${run_log}" >&2
    cat "${time_log}" >&2
    exit "${command_status}"
  fi
  ...
  grep "^RESULT name=yahoo-ad-${query}-${binary_mode} " "${run_log}"
  echo "YAHOO_AD_RSS_RESULT query=${query} mode=${mode} max_rss_bytes=${max_rss_bytes}"
...
echo
echo "Yahoo ad region matrix complete"
