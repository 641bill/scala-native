...
object NirPrimitives {
  ...
  final val RIFT_OPEN_ALLOC = 1 + RIFT_ALLOC

  final val USES_LINKTIME_INTRINSIC = 1 + RIFT_OPEN_ALLOC
...
class NirPrimitives(using ctx: Context) extends DottyPrimitives(ctx) {
  ...
  private def initNirPrimitives(using Context): ReadOnlyMap[Symbol, Int] = {
    ...
    defnNir.RuntimeRiftAllocator_allocateOpenHandleNoZero.foreach(addPrimitive(_, RIFT_OPEN_ALLOC))
